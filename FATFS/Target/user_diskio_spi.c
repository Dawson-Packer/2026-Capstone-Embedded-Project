...
//(Note that the _256 is used as a mask to clear the prescalar bits as it provides binary 111 in the
// correct position)
#define FCLK_SLOW()                                                                                \
    {                                                                                              \
        MODIFY_REG(SD_SPI_HANDLE.Instance->CR1, SPI_BAUDRATEPRESCALER_256,                         \
...
uint8_t SPI_Timer_Status() { return ((HAL_GetTick() - spiTimerTickStart) < spiTimerTickDelay); }
